...
void ArvBinBusca::emOrdem()
{
    auxemOrdem(raiz);
}
void ArvBinBusca::auxemOrdem(NoArv*p)
{
    if(p!=NULL)
    {
        auxemOrdem(p->getEsq());
...
int ArvBinBusca::contapares(int x)
{
    return auxcontapares(raiz,x);
}
int ArvBinBusca::auxcontapares(NoArv*p,int t)
{
    if(p==NULL)
        return 0;
    if(p->getInfo()!=t)
    {
        int c;
        if(p->getInfo()<t)
            c=auxcontapares(p->getDir(),t);
        else if(p->getInfo()>t){
            c=auxcontapares(p->getEsq(),t);
        }
        if(p->getInfo()%2==0)
        {
           return c+1;
